...
devtools::install()
```
*note* The debug flag in `tools/config.R` is turned off (hard coded) to get the faster rust `--release` builds. I don't think this affects the R part of package developement and is only needed due to rextendr.
